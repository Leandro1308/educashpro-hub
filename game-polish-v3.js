(function(){
  ...
  function makeQuestion(random,difficulty){
    ...
    if(kind===2){const pct=[10,20,25,50,75][pick(5)],baseValue=4*(5+pick(21));return[`${pct}% de ${baseValue}`,String(pct*baseValue/100)]}
  ...
  function makeChoices(correct,random){
    const right=normalize(correct),set=new Set([right]),fractionMatch=right.match(/^(-?\d+)\/(\d+)$/);
    if(fractionMatch){
      const n=Number(fractionMatch[1]),d=Number(fractionMatch[2]);
      ...
      if(Number.isFinite(n))shuffle([-3,-2,-1,1,2,3,4,-4],random).forEach(offset=>set.add(String(n+offset)));
    }
  ...
  function beep(ok){
    ...
    try{
      const AC=window.AudioContext||window.webkitAudioContext,ctx=beep.ctx||(beep.ctx=new AC()),osc=ctx.createOscillator(),gain=ctx.createGain();
      osc.frequency.value=ok?700:190;gain.gain.value=.03;osc.connect(gain);gain.connect(ctx.destination);osc.start();osc.stop(ctx.currentTime+.07);
    }catch{}
  }
  ...
  function renderMath(level,requested,tournament){
    ...
    function feedback(msg){const el=$("#mathFeedbackV3");if(!el)return;el.textContent=msg;el.classList.remove("show");void el.offsetWidth;el.classList.add("show")}
    function drawChoices(){const host=$("#mathOptionsV3");if(!host)return;host.innerHTML=choices.map(v=>`<button class="mathChoiceOption" data-answer="${esc(v)}">${esc(v)}</button>`).join("");host.querySelectorAll("[data-answer]").forEach(btn=>btn.onclick=()=>choose(btn))}
    function nextQuestion(){if(finished)return;question=makeQuestion(random,level);choices=makeChoices(question[1],random);y=62;locked=false;const q=$("#mathQuestionV3");if(!q)return finish();q.textContent=`${question[0]} = ?`;q.style.top=`${y}px`;drawChoices()}
    function loseLife(kind,selected){errors+=kind==="wrong"?1:0;misses+=kind==="miss"?1:0;combo=0;score-=1;lives=Math.max(0,lives-1);events.push(kind==="miss"?{miss:true}:{answer:selected});beep(false);hud();feedback(c("wrong",l));if(lives<=0){finish();return}setTimeout(nextQuestion,180)}
    function gainLife(){hits++;combo++;bestCombo=Math.max(bestCombo,combo);score+=1;lives+=1;events.push({answer:question[1]});beep(true);hud();const life=$("#mathLifeV3");life?.classList.remove("bump");if(life){void life.offsetWidth;life.classList.add("bump")}const laser=$("#mathLaserV3");laser?.classList.remove("fire");if(laser){void laser.offsetWidth;laser.classList.add("fire")}feedback(c("correct",l));setTimeout(nextQuestion,220)}
    ...
    function frame(now){
      ...
      if(!paused&&!locked){
        const q=$("#mathQuestionV3"),arena=$("#mathChoiceArena");
        if(!q||!arena){finish();return}
        const baseSpeed={basic:34,medium:44,advanced:41,nerd:47}[level]||40,factor={slow:.72,normal:1,fast:1.32}[speed]||1;
        y+=dt*(baseSpeed+Math.min(38,hits*10/65))*factor/1000;q.style.top=`${y}px`;
        if(y>arena.clientHeight-205){locked=true;loseLife("miss")}
      }
    ...
    async function finish(){
      ...
      if(active()){try{await suite.api?.("/api/games/history/save",{game:"math-space",difficulty:level,score,durationMs,hits,errors:errors+misses,bestCombo,stage:events.length})}catch{}}
      content().innerHTML=`<main class="gamePage"><section class="hero"><span class="eyebrow">${esc(st("gameOver",l))}</span><h1>⭐ ${score}</h1><p>${esc(st("record",l))}: <b>${record}</b> · ${esc(st("combo",l))}: x${bestCombo}</p></section><section class="socialActions"><button id="mathAgainV3" class="wideButton">${esc(c("playAgain",l))}</button><button id="mathLevelsV3" class="secondaryButton">${esc(c("levels",l))}</button></section></main>`;
      $("#mathAgainV3").onclick=()=>renderMath(level,l,null);$("#mathLevelsV3").onclick=()=>mathLevelSelector(l);
    }
    ...
    nextQuestion();hud();raf=requestAnimationFrame(frame);window.scrollTo({top:0,behavior:"auto"});
  }
  ...
  function findWordPath(word,cells,size){
    ...
    for(let r=0;r<size;r++)for(let col=0;col<size;col++)for(const[dr,dc]of dirs){
      const path=[];let ok=true;
      for(let i=0;i<word.length;i++){
        const rr=r+dr*i,cc=col+dc*i;
        if(rr<0||rr>=size||cc<0||cc>=size||cells[rr*size+cc]?.textContent!==word[i]){ok=false;break}
        path.push(cells[rr*size+cc]);
      }
      if(ok)return path;
    }
    return[];
  ...
  function polishWords(){
    const board=document.querySelector(".wordBoard"),chips=[...document.querySelectorAll(".wordChip")];
    if(!board||!chips.length)return;
    const cells=[...board.querySelectorAll(".wordCell")],size=Math.round(Math.sqrt(cells.length));
    if(size*size!==cells.length)return;
    chips.forEach((chip,i)=>{
      const color=WORD_COLORS[i%WORD_COLORS.length],word=chip.textContent.trim();
      chip.style.setProperty("--word-color",color);
      if(!chip.classList.contains("done")){chip.style.color=color;return}
      chip.style.color="";
      findWordPath(word,cells,size).forEach(cell=>cell.style.setProperty("--word-color",color));
    });
  }

  function pegMoves(from,pegSet,cellSet,triangle){
    const[fr,fc]=from.split(",").map(Number),dirs=triangle?[[0,2],[0,-2],[2,0],[-2,0],[2,2],[-2,-2]]:[[0,2],[0,-2],[2,0],[-2,0]];
  ...
  function polishPeg(){
    ...
    if(!board)return;
    const holes=[...board.querySelectorAll("[data-hole]")],cellSet=new Set(holes.map(h=>h.dataset.hole)),pegSet=new Set(holes.filter(h=>h.classList.contains("peg")).map(h=>h.dataset.hole)),triangle=board.classList.contains("pegTriangle");
    holes.forEach(h=>{h.classList.remove("movable");if(!h.classList.contains("selected"))h.classList.remove("target")});
    holes.filter(h=>h.classList.contains("peg")).forEach(h=>{if(pegMoves(h.dataset.hole,pegSet,cellSet,triangle).length)h.classList.add("movable")});
    const selected=holes.find(h=>h.classList.contains("selected"));
    if(selected)pegMoves(selected.dataset.hole,pegSet,cellSet,triangle).forEach(to=>board.querySelector(`[data-hole="${to}"]`)?.classList.add("target"));
  }
  ...
  function polishCross(){
    const grid=document.querySelector(".crossGrid");if(!grid)return;
    const hero=grid.closest(".gamePage")?.querySelector(".hero");
    if(hero&&!hero.querySelector(".crossManualBtn")){
      const b=document.createElement("button");b.type="button";b.className="crossManualBtn";b.textContent=`📘 ${c("manual",lang())}`;
      b.onclick=()=>simpleModal(c("manualTitle",lang()),c("manualSteps",lang()),lang());hero.appendChild(b);
    }
  }
  function polish(){polishSliding();polishCross();polishWords();polishPeg()}
  let polishQueued=false;
  function schedulePolish(){
    if(polishQueued)return;polishQueued=true;
  ...
  suite.launchGame=function(gameId,options={}){
    if(gameId==="math-space"){
      const l=lang(options.lang),level=options.tournament?.difficulty||options.level;
      return level?renderMath(level,l,options.tournament||null):mathLevelSelector(l);
    }
    const result=originalLaunch?.(gameId,options);
    schedulePolish();
    return result;
  };

  document.addEventListener("click",event=>{
    const play=event.target.closest?.('[data-play="math-space"]');
    if(play){setTimeout(()=>mathLevelSelector(lang()),0);return}
    const hint=event.target.closest?.("#pegHint");
    if(hint){
      const board=document.querySelector(".pegBoard");
      board?.classList.add("hinting");
      setTimeout(()=>board?.classList.remove("hinting"),1800);
    }
    if(event.target.closest?.(".pegBoard,.wordBoard,.crossGrid,.crossBank,[data-slide]"))schedulePolish();
  });

  const originalBoot=base.bootPublic?.bind(base);
  base.bootPublic=async function(params){
    if(String(params?.get?.("game")||"")==="math-space"){mathLevelSelector(params.get("lang"));return true}
    return originalBoot?originalBoot(params):false;
  };

  if(document.readyState==="loading")document.addEventListener("DOMContentLoaded",schedulePolish,{once:true});
  else schedulePolish();

  window.EduCashProGamePolishV3={renderMath,mathLevelSelector,polish:schedulePolish};
})();