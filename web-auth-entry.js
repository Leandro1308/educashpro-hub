(function(){
  ...
  const state={session:null,checking:true,ui:null,challenge:null,unsubscribe:null,unsubscribeModal:null,busy:false};
  ...
  function injectStyles(){if(document.getElementById("educashproWebAuthStyles"))return;const style=document.createElement("style");style.id="educashproWebAuthStyles";style.textContent=`
    ...
    .webAuthLayer{position:fixed;z-index:9999;inset:0;display:grid;place-items:end center;padding:18px;background:rgba(1,7,15,.72);backdrop-filter:blur(8px);transition:background .16s ease}
    .webAuthLayer.walletPickerOpen{background:transparent;backdrop-filter:none;pointer-events:none}
    .webAuthLayer.walletPickerOpen .webAuthSheet{opacity:0;visibility:hidden;transform:translateY(12px);pointer-events:none}
    .webAuthSheet{width:min(100%,520px);padding:20px;border:1px solid rgba(255,255,255,.1);border-radius:24px;background:#0d1b2d;color:#f7fbff;box-shadow:0 24px 70px rgba(0,0,0,.45);transition:opacity .14s ease,transform .14s ease}
  ...
  function closeLayer(){state.unsubscribeModal?.();state.unsubscribeModal=null;document.querySelector(".webAuthLayer")?.remove()}
  ...
  async function openLogin(){injectStyles();closeLayer();const layer=document.createElement("div");layer.className="webAuthLayer";layer.innerHTML=`<section class="webAuthSheet"><button class="webAuthClose" type="button" aria-label="Fechar">✕</button><span class="webMemberBadge">CONTA EDUCASHPRO</span><h2>Entrar com carteira TON</h2><p>A carteira será usada apenas para provar que este endereço pertence a você. Nenhuma transação é solicitada neste login.</p><div id="webAuthTonConnect"></div><div id="webAuthError" class="webAuthError"></div></section>`;document.body.appendChild(layer);layer.querySelector(".webAuthClose").onclick=closeLayer;
    try{
      ...
      state.unsubscribeModal?.();state.unsubscribeModal=state.ui.onModalStateChange?.(syncWalletPickerLayer)||null;
